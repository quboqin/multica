...
| `document` | Read and write documents: the tree, versioned body saves, and status |
| `collection`, `record` | Work with tables and their rows: fields, cells, and links to issues |
...
| | `search <query>` | Search issues | `--limit`, `--include-closed`, `--kind` (`task` or `doc`; every kind when omitted) |
| | `records <id>` | List the table rows that link to an issue | |
...
| `document` | `list` | List the document tree (titles and status, no bodies) | `--project` |
| | `get <document>` | Show a document with its body and revision | `--output markdown` prints the body alone |
| | `create` | Create a document (always starts as a draft) | `--title` (required), `--content` / `--content-stdin` / `--content-file`, `--parent`, `--project` |
| | `save <document>` | Save the body against the revision you read | `--expected-revision` (required), `--content` / `--content-stdin` / `--content-file`, `--title`; a stale revision is refused and nothing is written |
| | `move <document>` | Move within the tree | `--parent` / `--root`, `--before` / `--first` |
| | `status <document> <status>` | Submit for review, publish, or return to draft (`draft`, `reviewing`, `published`) | `--expected-revision` (required); people only — an agent's task token is refused |
| `collection` | `list/get/create/update/archive` | Manage tables | `create`: `--name`, `--project`; `update`: `--name`, `--title-name`. Anyone in the workspace can create a table; renaming, archiving and field changes are for its creator and workspace owners/admins. An agent's run acts for the person who owns its runtime and follows the same rule |
| | `field list/add/update/archive <table>` | Manage a table's fields | `add`: `--name`, `--type` (the nine property types plus `relation`), `--option` (repeatable), `--relation-to` (`issues` or a table; cannot change afterwards); `update`: `--add-option` appends options and keeps the rest, `--option` replaces the whole list and clears any option left out from every row |
| `record` | `list <table>` | List rows | `--search`, `--filter` (repeatable, `"Field=Value"`; `__none__` matches an empty cell), `--sort`, `--desc`, `--limit` (up to 100), `--cursor`, `--detail` (JSON only; field ids and stored values instead of names) |
| | `get/create/update/delete/restore <table> …` | Read and write rows | `create` / `update`: `--title`, `--set` (repeatable, `"Field=Value"`), `--unset`, `--expect` (only write while the cell still holds this value) |
| | `trash <table>` | List deleted rows (restorable for 30 days) | |
| | `link/unlink <table> <row>` | Link a row to issues or to other rows | `--field` (a relation field), `--to` (repeatable; an issue key, or a row id or exact title) |
| | `backlinks <table> <row>` | List the rows that link to this row | |
...
## Table imports and batches

CSV import validates all rows before writing (10,000 maximum). Batches accept up to 500 explicit record IDs; --expect-revision covers every row when used. Delete requires --yes; trash retention is 30 days. Archived tables and fields can be restored; field name conflicts and the 50-field limit return an error.

```bash
multica record import Customers --file customers.csv --dry-run
multica record import Customers --file customers.csv
multica record batch-update Customers --record <row-id> --set "Seats=25" --expect-revision "<row-id>=3"
multica record batch-delete Customers --record <row-id> --yes
multica record batch-restore Customers --record <row-id>
